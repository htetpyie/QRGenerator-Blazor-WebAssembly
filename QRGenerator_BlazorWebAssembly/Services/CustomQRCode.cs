...
public class CustomQRCode : AbstractQRCode
{
    public CustomQRCode(QRCodeData qrCodeData) : base(qrCodeData)
    {
    }
        ...
        Color? iconBackgroundColor = null)
    {
        ...
        for (int i = 0; i < num + num2; i += pixelsPerModule)
        {
            ...
            {
                SolidBrush brush =
                    (base.QrCodeData.ModuleMatrix[(j + pixelsPerModule) / pixelsPerModule - 1][
                        (i + pixelsPerModule) / pixelsPerModule - 1]
                        ? solidBrush
                        : solidBrush2);
        ...
        if (flag)
        {
            ...
            RectangleF rect = new RectangleF(num5 - (float)iconBorderWidth, num6 - (float)iconBorderWidth,
                num3 + (float)(iconBorderWidth * 2), num4 + (float)(iconBorderWidth * 2));
            RectangleF destRect = new RectangleF(num5, num6, num3, num4);
            SolidBrush brush2 =
                (iconBackgroundColor.HasValue ? new SolidBrush(iconBackgroundColor.Value) : solidBrush2);
    ...
    public GraphicsPath CreateRoundedRectanglePath(RectangleF rect, int cornerRadius)
    {
        ...
        graphicsPath.AddArc(rect.X + rect.Width - (float)(cornerRadius * 2), rect.Y, cornerRadius * 2, cornerRadius * 2,
            270f, 90f);
        graphicsPath.AddLine(rect.Right, rect.Y + (float)(cornerRadius * 2), rect.Right,
            rect.Y + rect.Height - (float)(cornerRadius * 2));
        graphicsPath.AddArc(rect.X + rect.Width - (float)(cornerRadius * 2),
            rect.Y + rect.Height - (float)(cornerRadius * 2), cornerRadius * 2, cornerRadius * 2, 0f, 90f);
        graphicsPath.AddLine(rect.Right - (float)(cornerRadius * 2), rect.Bottom, rect.X + (float)(cornerRadius * 2),
            rect.Bottom);
        graphicsPath.AddArc(rect.X, rect.Bottom - (float)(cornerRadius * 2), cornerRadius * 2, cornerRadius * 2, 90f,
            90f);
        graphicsPath.AddLine(rect.X, rect.Bottom - (float)(cornerRadius * 2), rect.X,
            rect.Y + (float)(cornerRadius * 2));
